...
var fields = map[string]meta{
	...
	"capture":                        {desc: "流量入口"},
	"capture.mode":                   {desc: "auto / socks：只开 SOCKS5；tproxy：Linux nftables 透明捕获（需 root）；tun：TUN 设备 + 用户态协议栈（目前仅 Linux，需 root）", enum: []string{"auto", "socks", "tproxy", "tun"}, def: "auto"},
